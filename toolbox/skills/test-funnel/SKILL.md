...
把整理過的 AC 落地成 **TC 表格文件**，格式用 `testcase-template.md`（萃取自 canonical `freezer.md`）。
...
>
> canonical 對照：freezer **TC-04.2「到上限後放大 disabled」仍是 `✅`**——到上限要跑真動畫 `zoomInUntilSettled` + 等 `checkButtonState` 的 postDelayed 才翻 disabled，是真機 async 路徑；但純 `nextZoomScale` / clamp 數學只留 unit。**反例**：碼錶 pause/resume 累加只是注入時鐘的算術、無真動畫/手勢 → unit 就夠，別標 `✅`。
...
- 每寫完一個測試，回填對應列的符號（`✅` / `unit:` / `👁`）。
- 補上文件尾的「自動化盤點」段：instrumented N case 覆蓋哪些 TC、unit 守哪些、哪些維持手動及理由、已知 quirk。
...
| 重編 / 回收 TC 編號 | 只新增不重編，廢棄標 `(Deprecated)`，避免外部引用斷裂 |

---

## Canonical 範例

- `test_case/freezer.md`（edu-vbos-finch，VB-579 / VB-882）— 本 skill 格式與決策樹的來源；含完整三層分流盤點。
- `ZoomActivityInstrumentedTest.kt` — `instrumented-harness.kt` 的真實出處（IFP41 全綠 10 case）。

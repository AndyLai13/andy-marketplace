...
- 回報給 user：
  ...
  - 遇到的摩擦點（若有非預期）
  - 下一步：「準備好開始 PR #1 Foundation 嗎？」— 若實作走 git worktree，附帶提醒：先過 §Worktree env-file gate（symlink `local.properties` / `keystore.properties` / `MVBA_PlatForm.jks`，缺 keystore 會靜默簽成 debug key、上機更新才爆 `UPDATE_INCOMPATIBLE`）
...
### Worktree env-file gate（簽章 — 任何走 git worktree 實作的 PR 都適用）

任一 Task（Foundation / slice / Final）若在 **git worktree** 裡實作（例如 `/toolbox:start-task` 或 superpowers:using-git-worktrees），**第一次 build 之前**必須先把 gitignore 的 env / 簽章檔從主 checkout symlink 過來（用 symlink、不要 copy — copy 會 drift）：

```bash
MAIN=$(git worktree list --porcelain | head -1 | cut -d' ' -f2)   # 主 checkout 路徑
for f in local.properties keystore.properties MVBA_PlatForm.jks; do
  [ -e "$MAIN/$f" ] && [ ! -e "$f" ] && ln -s "$MAIN/$f" "$f"
done
ls -lL local.properties keystore.properties MVBA_PlatForm.jks   # 驗證三檔都解析得到
```

⚠️ 為什麼是 **gate** 不是小提示：缺 `local.properties` 會立刻 fail（SDK path 錯誤），但缺 `keystore.properties` / `MVBA_PlatForm.jks` **不會讓 build 失敗** — Gradle 靜默 fallback 用 **debug key** 簽章。APK 照樣 build、全新安裝也裝得起來，直到上機更新既有 release 簽章的安裝才爆：`INSTALL_FAILED_UPDATE_INCOMPATIBLE`。靜默失敗 + 晚期才發現 = 必須前置檢查。

（上面檔案清單是 edu-vbos-finch 的；其他專案 symlink `signingConfigs` 引用到的檔案 + `local.properties`。）

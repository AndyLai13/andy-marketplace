---
name: finalize-doc-lifecycle
description: Use when an implementation plan has finished shipping and the draft + plan files need to be absorbed into canonical specs and deleted — the draft → plan → ship → spec lifecycle's closing step. Triggers — user says "本任務完成" / "task complete" / "收尾 lifecycle" / "plan 跑完了把文件收掉"; drafts/plans in docs/superpowers/ accumulating un-absorbed after features ship. This is the executor counterpart to dev-bootstrap (which only scaffolds the lifecycle). Gated on user QA confirmation — never auto-run.
---

# finalize-doc-lifecycle

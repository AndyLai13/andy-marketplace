---
name: dual-loop-flow
description: 以縱切（vertical slice）+ 雙循環 TDD（double-loop TDD，亦稱 outside-in TDD）工作流規劃多 PR feature。適用情境：feature 需要 5+ PR、AC 已凍結、trunk-based 環境。OPSX / Figma / Jira 有就用、沒有則優雅降級（不卡住）。Trigger：`/dual-loop-flow <feature-name>` 或 `/dual-loop-flow <parent-ticket>`。
---
...
✅ User 透過 `/dual-loop-flow` 觸發，或說「拆 multi-story feature」/「規劃 multi-PR」等
